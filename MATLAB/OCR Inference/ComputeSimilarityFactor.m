...
for ii = 1:(K*K)
    state_assignment = IndexToAssignment(ii, [K, K]);
    if state_assignment(1) == state_assignment(2) % For all the diagonal entries assign similarity;
        factor.val(ii) = ImageSimilarity(images(i).img, images(j).img);
    end
end
