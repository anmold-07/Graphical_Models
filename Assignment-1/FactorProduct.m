...
if ~isempty(dummy)                                                                  % not empty
	...
	assert(all(A.card(iA) == B.card(iB)), 'Dimensionality mismatch in factors');    % Cardinalty of respective inteseted variables
end
...
[dummy, mapA] = ismember(A.var, C.var);    % Creating Maps
[dummy, mapB] = ismember(B.var, C.var);    % Creating Maps

% Set the cardinalities of variables in C
